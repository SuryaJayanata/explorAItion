<div class="comment bg-white/5 rounded-lg p-4 border border-white/10 {{ $level > 0 ? 'ml-6' : '' }}" 
    ...
    <div class="flex items-start justify-between mb-3">
        <div class="flex items-center space-x-2">
            ...
            <div>
                <div class="flex items-center space-x-2">
                    ...
                    @if($komentar->user->id_user == $tugas->kelas->id_guru)
        ...
        <form action="{{ route('kelas.tugas.komentar.destroy', [$kelas->id_kelas, $tugas->id_tugas, $komentar->id_komentar]) }}" 
    ...
    <div id="replies-{{ $komentar->id_komentar }}" class="mt-3 space-y-3 border-t border-white/10 pt-3 hidden">
        @foreach($komentar->replies as $reply)
            @include('tugas.partials.comment', ['komentar' => $reply, 'level' => $level + 1])
        @endforeach
    </div>
    @endif
</div>